...
class CameraManager(object):
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: CameraManager._parse_image_sensor(weak_self,self.sensors[index], image))
        if notify:
    ...
    def _parse_image_sensor(weak_self, sensor, image):
        ...
        if sensor[0].startswith('sensor.lidar'):
            ...
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
            image.save_to_disk('out%s_%s:/%08d' % (filename ,sensor[0].split('.')[2],image.frame))
